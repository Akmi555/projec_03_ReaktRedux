...
export const randomJokesSlise = createAppSlice({
  name: "RANDOM_JOKES",
  initialState: randomJokesInitialState,
  // 1. middleware создается в объекте reducers вместе с обычными редьюсерами
  reducers: create => ({
    ...
    // 2-й аргумент - объект c 3-мя методами, которые обрабатывают результат 
    ...
    getJoke: create.asyncThunk(
      async (arg, thunkApi) => {
        try {
         ...
         //чтобы получить их в обработчике ()агдашддув
          return result.data
        } catch (error) {
...
export const randomJokesSliceActions = randomJokesSlise.actions
export const randomJokesSliceSelectors = randomJokesSlise.selectors
